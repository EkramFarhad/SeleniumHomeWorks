...
public class DynamicTable {
    public static void main(String[] args) throws InterruptedException {
        ...
        //get the colum
        List<WebElement> colum = driver.findElements(By.xpath("//table[@id='resultTable']/tbody/tr/td[4]"));
        for (int i = 0; i < colum.size(); i++) {
            String text = colum.get(i).getText();
            if (text.equalsIgnoreCase("Alfonso")) {
                // System.out.println(text + i);
                List<WebElement> row = driver.findElements(By.xpath("//table[@id='resultTable']/tbody/tr[" + (i + 1) + "]/td[4]"));
